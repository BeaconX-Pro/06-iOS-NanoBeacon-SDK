...
@implementation MKNNBScanSearchButtonModel
@end

@interface MKNNBScanSearchButton ()
...
@implementation MKNNBScanSearchButton
...
- (void)clearButtonPressed{
    ...
    if ([self.delegate respondsToSelector:@selector(nnb_scanSearchButtonClearMethod)]) {
        [self.delegate nnb_scanSearchButtonClearMethod];
    }
}

- (void)searchButtonPressed{
    if ([self.delegate respondsToSelector:@selector(nnb_scanSearchButtonMethod)]) {
        [self.delegate nnb_scanSearchButtonMethod];
    }
}

#pragma mark - setting
- (void)setDataModel:(MKNNBScanSearchButtonModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKNNBScanSearchButtonModel.class]) {
        return;
    }
...
- (UIImageView *)searchIcon{
    if (!_searchIcon) {
        ...
        _searchIcon.image = LOADICON(@"MKNanoBeacon", @"MKNNBScanSearchButton", @"nnb_searchGrayIcon.png");
    }
    return _searchIcon;
}
...
- (UIButton *)clearButton{
    if (!_clearButton) {
        ...
        [_clearButton setImage:LOADICON(@"MKNanoBeacon", @"MKNNBScanSearchButton", @"nnb_clearButtonIcon.png") forState:UIControlStateNormal];
